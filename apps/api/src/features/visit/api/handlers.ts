...
const createVisit: RequestHandler = catchAsync(async (req, res) => {
  const visit = await services.createVisit(req.body);
  res.status(201).json(visit);
});

const getVisit: RequestHandler = catchAsync(async (req, res) => {
  const visitId = req.params.id as string;
  const visit = await services.getVisit(visitId);
  res.status(200).json(visit);
});

const getAllVisits: RequestHandler = catchAsync(async (req, res) => {
  const visits = await services.getAllVisits();
  res.status(200).json(visits);
});

const addEvidence: RequestHandler = catchAsync(async (req, res) => {
...
const generateReport: RequestHandler = catchAsync(async (req, res) => {
  const visitId = req.params.id as string;
  const report = await services.generateReport(visitId);
...
const removeEvidence: RequestHandler = catchAsync(async (req, res) => {
  const evidenceId = req.params.id as string;
  await services.removeEvidence(evidenceId);
  res.status(204).send();
});

export { createVisit, getAllVisits, getVisit, addEvidence, generateReport, getReport, updateStatus, removeEvidence };